...
# modify:2013年12月06日 星期五 11时56分49秒
# 功能描述：用于备份文件的脚本。 注意这里使用了绝对压缩路径 如果解压时不想使用绝对路径压缩则不要使用P选项
# bug
## 目前不能处理带有空格的行
## -错误：刚开始你判断$1是否以-开头，如果不是就报错退出。所以如果你以后想在该脚本中使用单独的参数。就直接被这个if语句退出了。也即这个脚本只能使用选项而不能使用参数。
...
## 使用方式二：用户自己指定配置文件路径
### 配置路径可放在任意路径下，但格式要与默认配置文件一致。最好把默认配置文件复制过去后修改参数
### 确保该脚本具有x权限 chmod u+x backup.sh
